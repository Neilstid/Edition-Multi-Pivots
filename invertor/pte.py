...
from invertor.utils import gram_schmidt_process, read_image
...
class PivotalTuningEdition:
    ...
    def synthethise_with_multi_directions(
        self, 
        pseudo_target: List[Union[str, Image.Image, np.array, torch.Tensor, None]],
        # Invertion params
        invertion_iteration: int = 0, 
        loss_function: Callable = None,
        initial_latent_function: Callable = None, 
        ...
        tuning_iteration: int = 1500,
        learning_rate_tuning: float = 3e-4,
        direction_type: Literal["Double_Invertion", "PCA", "Random", "Random_Local", "Chosen_Direction"] = "Double_Invertion",
        direction: Union[torch.Tensor, np.array] = None
    ):
        
        # Initialize the algorithm --------------------------------------------
        num_direction = len(pseudo_target)
        
        # Invert the target and PTI -------------------------------------------
        self.invert(
            invertion_iteration=invertion_iteration, loss_function=loss_function,
            initial_latent_function=initial_latent_function,
            initial_latent_function_kwargs=initial_latent_function_kwargs,
        ...
        if direction_type == "Double_Invertion" or (direction_type == "Chosen_Direction" and direction is None):
            ...
            pca.fit([
                self._generator.mapping(
                    torch.from_numpy(np.random.randn(10000, 512)).cuda(), 
        ...
        # Orthogonal direction estimations ------------------------------------
        dir_origin = self._direction.detach().requires_grad_(False)
        all_dir = []
        ...
        for i in range(1, 100):
            pca_fill_direction = np.zeros(512)
            pca_fill_direction[i] = 1
            all_dir.append(
                torch.from_numpy(pca.inverse_transform(
                    w_transform.copy() + pca_fill_direction
                )).cuda().requires_grad_(False).to(dir_origin.dtype).reshape(dir_origin.shape)
            )

        # all_dir += [dir_origin]
        all_dir += []
        lb, ub = torch.min(dir_origin), torch.max(dir_origin)
        for n in range(num_direction):
            dir_temp = torch.distributions.Uniform(lb, ub).sample(dir_origin.shape)
            # idx = torch.randperm(dir_origin.shape[-1])
            # dir_temp = dir_origin[..., idx]
            all_dir.append(dir_temp)

        # Orthogonal directions
        orth_dir = (
            torch.stack(gram_schmidt_process(all_dir)) / torch.norm(dir_origin)
        ).requires_grad_(False).to(self._device)[-num_direction:]
        
        # Initialize the tuning phase -----------------------------------------
        lpips_loss = LpipsLoss()

        self._generator_pti = copy.deepcopy(self._generator).train().requires_grad_(True).to(self._device)
        optimizer = tune_optimizer(self._generator_pti.parameters(), lr=learning_rate_tuning, **opt_args)

        pseudo_targets = [read_image(p).squeeze(0) for p in pseudo_target]
        target = read_image(self._target).squeeze(0)

        c = torch.zeros([1, 0], device=self._device)

        # Direction algorithm -------------------------------------------------
        for i in tqdm(range(tuning_iteration)):
            pseudo_pivot = self._pivot - orth_dir[i % num_direction, ...]
            pgt = pseudo_targets[i % num_direction]

            latent_pivots = [
                self._space_modifier(self._pivot).squeeze(0).detach().requires_grad_(False),
                self._space_modifier(pseudo_pivot).squeeze(0).detach().requires_grad_(False)
            ]
            latent_pivots = torch.stack(latent_pivots, dim=0).requires_grad_(False).to(self._device)

            targeted_images = torch.stack([target, pgt])

            # Synth images from opt_w.
            synth_image = self._generator_pti(latent_pivots, c, **self._generator_kwargs)
            # track images
            synth_image = (synth_image.clamp(-1, 1) + 1) / 2

            # MSE loss
            mse_loss = torch.nn.functional.mse_loss(targeted_images, synth_image, reduction="mean").sum()
            lpips_loss_value = lpips_loss(targeted_images, synth_image).sum()
        ...
        # End the algorithm ---------------------------------------------------
        self._generator_pti.requires_grad_(False)
        self._direction = orth_dir[:, 0, ...]
