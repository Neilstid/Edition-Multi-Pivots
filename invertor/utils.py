...
def project(u, v):
    return (torch.dot(u.flatten(), v.flatten()) / torch.dot(u.flatten(), u.flatten())) * u


def gram_schmidt_single(u, v):
    return v - project(u, v)
...
def gram_schmidt_process(v):
    ...
    for vk in v:
        uk.append(
            vk - (torch.sum(torch.stack([project(uj, vk) for uj in uk]), dim=0) if len(uk) > 0 else torch.zeros_like(vk).to(vk.device))
        )
